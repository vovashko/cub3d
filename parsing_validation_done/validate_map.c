...
static void initialize_rows(char **map, int *i, char **row_on_top, char **row_on_bottom)
{
    *row_on_top = NULL;
    *row_on_bottom = NULL;
    if (i > 0)
...
static bool validate_edge_rows(char *curr_row, char *row_on_top, char *row_on_bottom, int i)
{
...
static bool handle_row_validation(char *curr_row, char *row_on_top, char *row_on_bottom)
{
...
bool validate_map_enclosure(char **map)
{
    ...
    while (map[i])
    {
        ...
        if (!validate_edge_rows(curr_row, row_on_top, row_on_bottom, i) || !handle_row_validation(curr_row, row_on_top, row_on_bottom))
            return false;
        i++;
    }
    return true;

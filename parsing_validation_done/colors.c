...
static int parse_color_component(char *line, int *index, const char *component_name)
{
    int value;
    ...
    if (line[*index] == ',' && ft_strncmp(component_name, "blue", 5) != 0)
        ...
        handle_error("Invalid color format: expected ',' after a color component");
    return value;
}

static void validate_color_component(char *line, int *color, const char *component_name, int *index)
{
    if (*color == -1)
    {
